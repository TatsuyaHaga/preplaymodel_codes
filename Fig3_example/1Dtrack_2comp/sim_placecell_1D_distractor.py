...
for t in range(1, sim_len+1):
    ...
    spont_distractor=spont_distractor+samp_pitch*(-spont_distractor/tau_distractor+distractor_noise_amp*numpy.random.randn(Ninput))
    
    #activity
    if moving:
        theta_osc=numpy.sin(2.0*numpy.pi*time_ms/1000.0*7.0)
        data[:NE]=theta_amp*theta_osc
        data[NE:NE+Ninput]=isinput*(theta_amp_input*theta_osc+theta_bias_input+placecell_func(x, y, max_rate_place, width_place))+isdistractor*(theta_amp_input*theta_osc+theta_bias_input+spont_distractor)
        if x<DGregion:
            DGsave=1
        ...
        data[:NE]=0.0
        data[NE:NE+Ninput]=isinput*spont_input+isdistractor*spont_input
