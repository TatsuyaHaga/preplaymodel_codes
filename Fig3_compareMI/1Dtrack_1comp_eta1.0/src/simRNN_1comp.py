...
for ext_input_str in csv_ext:
    ...
    Iinput=Iinput+time_pitch*(-Iinput/tauL+ext_input[NE:NE+Ninput])
    
    #activity
    inhinput_dnd=numpy.zeros(NE)
    Idnd=WEdnd@PSCinput+WEdndNMDA@PSCinputNMDA
    Isom=WEsom@PSCrecur+WEsomNMDA@PSCrecurNMDA-WIsom*Iinh-WIsomGABAB*IinhGABAB+Iext_som+Idnd
    x=sigmoid(Isom)
    y=numpy.zeros(NE)
    z=phi*x
    ...
    thetaEsom=c0*Ex*Ex
    deltaWEsom=deltaWEsom+time_pitch*(-deltaWEsom+numpy.outer(x*(x-thetaEsom)*(1.0-x), PSCrecur))/taudeltaW
    ...
    deltaWEdnd=deltaWEdnd+time_pitch*(-deltaWEdnd+numpy.outer(x*(x-thetaEsom)*(1.0-x), PSCinput))/taudeltaW
    ...
    if time_ms%save_pitch==0:
        ...
        temp=numpy.hstack([time_sec, Idnd])
