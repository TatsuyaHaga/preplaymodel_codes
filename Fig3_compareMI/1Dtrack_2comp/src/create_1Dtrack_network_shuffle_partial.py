...
dnd_shuffle_prob=float(sys.argv[3])
som_strength=float(sys.argv[4])

#soma exc synapse
EEmax=18.0*som_strength
EEwidth=5.0
EEbias=0.0
...
for i in range(exc_neuron_num):
    shuffle_arr=numpy.arange(input_neuron_num)<input_neuron_num*dnd_shuffle_prob
    numpy.random.shuffle(shuffle_arr)
    WEE[i,shuffle_arr]=numpy.random.permutation(WEE[i,shuffle_arr])
WEENMDA=WEE*AMPA_NMDA_ratio
