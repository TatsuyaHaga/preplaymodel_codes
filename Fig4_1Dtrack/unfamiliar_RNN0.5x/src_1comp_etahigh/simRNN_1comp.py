...
for ext_input_str in csv_ext:
    ...
    y=Idnd
    x=sigmoid(Isom+Idnd+Iext_som)#+beta_som*yprev+Iext_som)
    z=phi*x#z=(1.0+gamma*y)*phi*x
    ...
    thetaEsom=c0*Ex*Ex
    deltaWEEsom+=time_pitch*(-deltaWEEsom+numpy.outer(x*(x-thetaEsom)*(1.0-x), PSCrecur))/taudeltaW

    #WEEdnd plasticity
    WEEdnd+=time_pitch*(etaEdnd*deltaWEEdnd-eta_Wdecay*WEEdnd)
    WEEdnd[WEEdnd<Wmin]=Wmin
    #thetaEdnd=c0*Ey*Ey
    deltaWEEdnd+=time_pitch*(-deltaWEEdnd+numpy.outer(x*(x-thetaEsom)*(1.0-x), PSCinput))/taudeltaW
