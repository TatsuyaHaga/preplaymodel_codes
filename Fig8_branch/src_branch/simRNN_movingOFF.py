...
for ext_input_str in csv_ext:
    ...
    #WEEsom plasticity
    WEEsom+=time_pitch*(deltaWEEsom-eta_Wdecay*WEEsom)
    WEEsom[WEEsom<Wmin]=Wmin
    WEEsom[WEEsom_diagonal]=0.0
    thetaEsom=c0*Ex*Ex
    deltaWEEsom+=time_pitch*(-deltaWEEsom+etaEsom*moving*numpy.outer(((1.0-alpha_som)*x*(x-thetaEsom)+alpha_som*x*y)*(1.0-x), PSCrecur))/taudeltaW

    #WEEdnd plasticity
    WEEdnd+=time_pitch*(deltaWEEdnd-eta_Wdecay*WEEdnd)
    WEEdnd[WEEdnd<Wmin]=Wmin
    thetaEdnd=c0*Ey*Ey
    deltaWEEdnd+=time_pitch*(-deltaWEEdnd+etaEdnd*moving*numpy.outer(((1.0-alpha_dnd)*y*(y-thetaEdnd)+alpha_dnd*x*y)*(1.0-y), PSCinput))/taudeltaW

    #WEIdnd plasticity
    WEIdnd+=time_pitch*(deltaWEIdnd-eta_Wdecay*WEIdnd)
    WEIdnd[WEIdnd<Wmin]=Wmin
    deltaWEIdnd+=time_pitch*(-deltaWEIdnd+etaIdnd*moving*numpy.outer(((1.0-alpha_dnd)*y*(y-thetainh)+alpha_dnd*x*y)*(1.0-y), Idndinh))/taudeltaW
