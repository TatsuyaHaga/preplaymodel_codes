...
grid_coef=3.0
def gridcell_func(X,Y,max_rate,phase,width):
    return max_rate*numpy.exp(grid_coef*(numpy.cos(2.0*numpy.pi*X/width-phase)-1.0))
...
#grid cell
max_rate_grid=5.0
phase_grid=2.0*numpy.pi*numpy.random.rand(Ninput)
width_grid=0.2+0.4*numpy.random.rand(Ninput)
...
for t in range(1, sim_len+1):
    ...
    if moving:
        ...
        Iinput=theta_amp*(0.5*theta_osc-0.5)+isinput*gridcell_func(x, y, max_rate_grid, phase_grid, width_grid)+isdistractor*spont_distractor
        ...
        data[:NE]=Inoise[:NE]
        Iinput=isinput*gridcell_func(x, y, max_rate_grid, phase_grid, width_grid)+isdistractor*spont_distractor
        data[NE:NE+Ninput]=sigmoid(amp_rest*Iinput+Inoise[NE:NE+Ninput])
